...
func AddCategory(c *fiber.Ctx) error {
	...
	insertResult, err := database.ProductCollection.InsertOne(context.Background(), category)
...
func GetAllCategories(c *fiber.Ctx) error {
	...
	layer1Filter := bson.M{"parent_id": nil}

	cursor, err := database.ProductCollection.Find(context.Background(), layer1Filter)
...
func EditCategory(c *fiber.Ctx) error {
	...
	update := bson.M{
		...
		"titdescriptionle": updatable.Description,
		"link":             updatable.Link,
		"theme":            updatable.Theme,
		"details":          updatable.Detail,
	}
