          ...
          {orderCart.orders.map((order, index) => (
            ...
            >
              ...
              <div className="sm:col-span-1">
                <img src={order.product.picture} alt={order.product.productTitle} />
              </div>
  
              {/* Product details */}
              <div className="sm:col-span-5 mt-4 sm:mt-8">
                <p>{order.product.productTitle}</p>
                <div className="flex mt-5">
                  <div
                    className="w-5 h-5 rounded-full border border-grey-border ml-3"
                    style={{ backgroundColor: order.product.color.hex }}
                  />
                ...
                <div className="flex mt-3">
                  <svg
                    ...
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className="size-6 text-grey-dark ml-3"
                  >
                    <path
                      strokeLinecap="round"
              ...
              <div className="sm:col-span-5 self-center">
                <p className="text-3xl">
                  {order.product.price} <span className="text-sm text-primary-color">تومان</span>
                </p>
              </div>
            </div>
          ))}
          ...
          <div className="bg-white p-5 rounded-lg border border-grey-border">
            <div className="flex justify-between">
              ...
              <p className="text-2xl">
                {finalPrice}
                <span className="text-primary-color text-sm mx-2">تومان</span>
              </p>
            </div>
            ...
            <p className="mb-5 text-grey-dark">
              <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6 inline text-primary-color ml-2"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
            ...
            <button
              onClick={() => {
                user != null ? manage.push("/orders/shipping") : manage.push("/users/signIn");
              }}
              className="mt-10 bg-primary-color w-full text-white rounded-lg p-3"
            >
